...
function renderProjects(searchTerm = '') {
    ...
    if (active.length > 0) {
        html += '<div class="project-group-label">PROJELER</div>';
    ...
    if (archived.length > 0) {
        html += '<div class="project-group-label" style="margin-top: 20px;">ARCHIVE</div>';
...
function renderTasks() {
    ...
    if (!app.currentProject) {
        container.innerHTML = '<div class="empty-state"><h3>PROJE SECILMEDI</h3><p>Sol taraftan bir proje secin</p></div>';
        return;
    }
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<div class="empty-state"><h3>BOS</h3><p>Sag alttaki + butonuna tiklayin</p></div>';
        return;
    }
...
function renderTask(task) {
    const typeLabels = { task: 'GOREV', plan: 'PLAN', note: 'NOT', reminder: 'HATIRLATMA' };
    ...
    if (task.checklist && task.checklist.length > 0) {
        ...
        html += `<input type="text" class="checklist-input" placeholder="+ Alt gorev ekle..." data-task-id="${task.id}">`;
        html += '</div>';
    } else {
        html += `<div class="task-checklist"><input type="text" class="checklist-input" placeholder="+ Alt gorev ekle..." data-task-id="${task.id}"></div>`;
    }
